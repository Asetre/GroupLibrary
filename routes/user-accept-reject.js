...
module.exports = function(router) {
    ...
    router.post('/borrow-request/accept/:bookId/:requestId/:borrowerId', (req, res) => {
        ...
        //Remove the request
        const updateOwner = Users.findOneAndUpdate({_id: req.user._id}, {$pull: {borrowRequests: {_id: req.params.requestId}}}, {safe: true, multi: true}).exec()
            .then((user) => user)
        Promise.all([
            findBorrower,
            updateOwner
            ...
            .then((data) => {
                ...
                const book = data[1].books.id(req.params.bookId)
    ...
    router.post('/return-request/approve/:bookId/:borrowerId/:returnId', (req, res) => {
        const findOwner = Users.findOne({_id: req.user._id})
        const findBorrower = Users.findOne({_id: req.params.borrowerId})
        const removeReturn = Users.findOneAndUpdate({_id: req.user._id}, {$pull: {bookReturns: {_id: req.params.returnId}}}, {safe: true, multi: true})
        ...
        Promise.all([
            ...
            .then((data) => {
                ...
                const book = owner.books.id(req.params.bookId)
